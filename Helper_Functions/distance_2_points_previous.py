...
if __name__ == '__main__':	
	...
	files=glob.glob(args.file_path + '*.pdb')
	...
	for file in files:
		...
		for i in inFile:
			...
			if(i[0:4].rstrip())=="ATOM" \
				and (i[13:15].rstrip())=="CA"\
				and (i[16]=='A'or i[16]==' ') \
				and i[17:20]!= "UNK" \
				and i[21]== df_dict[ntpath.basename(file)[:4]]\
				and (i[16:20].strip() == aa1 or i[16:20].strip() == aa2):
		...
		for a in a_coords:
			for b in b_coords:
				if a[1] != b[1]:
					...
					result.append((a[0], a[1], b[0], b[1], a[2], a[3], a[4], b[2], b[3], b[4], \
						get_distance(a_coord,b_coord)))
