...
parser.add_argument('--aa0', '-aa0', metavar='aa0', \
    default='LYS', \
    help='aa0.')
parser.add_argument('--pos0', '-pos0', metavar='pos0', \
    default='10', \
    help='pos0.')
parser.add_argument('--aa1', '-aa1', metavar='aa1', \
    default='GLU', \
    help='aa1.')
parser.add_argument('--pos1', '-pos1', metavar='pos1', \
    default='12', \
    help='pos1.')
parser.add_argument('--aa2', '-aa2', metavar='aa2', \
    default='PHE', \
    help='aa2.')
parser.add_argument('--pos2', '-pos2', metavar='pos2', \
    default='11', \
    help='pos2.')
	...
	for file in triplet_files:
		with open(file) as f:
			for line in f.readlines():
				values = line.split('\t')
				#print(values)
				if (args.aa0 == values[1].strip() and args.pos0 == values[2].strip() and 
					args.aa1 == values[3].strip() and args.pos1 == values[4].strip() and 
					args.aa2 == values[5].strip() and args.pos2 == values[6].strip() ): 
					search_lines[file] = line
	print(search_lines)
	pd.DataFrame(search_lines.items()).to_csv(os.path.join(args.path, args.sample_name, args.bin_folder, 'search_lines_{}'.format(
		args.aa0,args.pos0,args.aa1,args.pos1,args.aa2,args.pos2)))

	for file in triplet_files:
		values_k1 = search_lines[file].split('\t')
