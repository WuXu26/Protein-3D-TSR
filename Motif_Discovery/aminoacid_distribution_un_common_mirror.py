...
def merge_key_triplet_file(key_file, triplet_file, file_name, is_freq_groups, delim, is_header):
	...
	s = time.time()
	use_col_names = ['key', 'aa0', 'aa1', 'aa2', 'theta', 'theta1', 'distance']
	if not key_file:
	...
	if is_header == 0:
		df_triplets = pd.read_csv(triplet_file , \
					...
					'theta': np.dtype(float), 'theta1': np.dtype(float), 'distance': np.dtype(float)}, \
	...
	print('triplets:', len(df_triplets))
		...
		if fileType == 'full':
			df = pd.read_csv(os.path.join(args.path, \
				...
				'{}.triplets_{}'.format(ntpath.basename(file)[:4], args.setting)) , \
					...
					'z1', 'x2', 'y2', 'z2'])
			print(df.head(5))
			# print("calculate_statistics- full")
...
def generate_common_key_files(all_keys_files, outFolder):
	...
	if os.path.exists(os.path.join(args.path, args.sample_name, args.setting, \
						'common_keys')):
		print('generate_common_key_files() : Common Key files already exist from a previous program.Check this flow.')
		...
		merged_files = glob.glob(os.path.join(args.path, args.sample_name, args.setting, '*.triplets*'))
		for file in merged_files:
			#If reading with pandas
			if iskeys_file == 1:
				start = time.time()
				df = pd.read_table(file, delimiter = delim,  \
					usecols = ['key', 'aa0', 'aa1', 'aa2', 'theta', 'distance'], \
					dtype = {'key': np.dtype(int), 'aa0': str, 'aa1': str,  'aa2': str, \
					'theta': np.dtype(float), 'theta1': np.dtype(float), 'distance': np.dtype(float)},\
					...
					'classT', 'theta', 'theta1', 'classL', 'distance', 'x0', 'y0', 'z0', 'x1', 'y1',\
					'z1', 'x2', 'y2', 'z2'])	
				# df = pd.read_table(file, delimiter = delim, header = 0,  \
				# 	usecols = use_col_names, \
	...
	if not un_common_key_files:
		...
		for file in all_keys_files:
			...
			c_keys = list(pd.read_table(c_file, delimiter = ',', header = 0,  \
					...
					'aa1': str,  'aa2': str, 'theta': np.dtype(float), 'theta1': np.dtype(float),\
