...
def search_by_key(fileName, file, keys, iskeys_file):
	...
	print (fileName, line_count, len(set(df['key'].values)), len(df['key'].values))
...
def identify_pattern_by_regex(files,column_names, patterns):
	...
	return ntpath.basename(file)[:8], matched
...
def identify_pattern_by_normal(file, pattern,count_of_x, group):
	...
	if float(os.path.getsize(file)/ 2**30) > 4 :
		...
		return ntpath.basename(file)[:8], ['File Size too big']
		#all_pattern = generate_sequences_from_triplets_by_FR(file)	
...
def calculate_low_less15_freq_from_common_keys(outFolder, req_low_freq, req_max_dist):
	...
	for file in common_keys_files:
		...
		ckeys_freqs_df.to_excel(writer,sheet_name=ntpath.basename(file)[:8])
		#Low Frequency Common Keys
		low_freqs = ckeys_freqs_df[ckeys_freqs_df['freq'] <= req_low_freq]
		low_freqs.to_excel(writer_low_freq,sheet_name=ntpath.basename(file)[:8])
		...
		df_distance_less15.to_excel(writer_dist_less15,sheet_name=ntpath.basename(file)[:8])
		summary.append((ntpath.basename(file)[:8], len(set(df_low_freqs_triplets['key'].values)),len(df_low_freqs_triplets['key'].values), \
			len(set(df_distance_less15['key'].values)), len(df_distance_less15)))
		print (ntpath.basename(file)[:8], len(set(df_low_freqs_triplets['key'].values)),len(df_low_freqs_triplets['key'].values), \
...
def process_files(file, req_aacds):
	...
	req_pos = re.findall(r'\d+', samples_file.set_index('protein').\
		loc[ntpath.basename(file)[:8],  'pattern'])
	#req_pos = []
	...
	records.to_csv(os.path.join(args.path, args.sample_name, \
			args.setting, 'common_keys', \
	 		ntpath.basename(file)[:8] + '_'+ "_".join(req_aacds) + '.csv'))
	print (ntpath.basename(file)[:8], pd.to_numeric(records['theta']).mean(),\
	 pd.to_numeric(records['distance']).mean(), pattern_theta_1, pattern_theta_2,\
	 pattern_dist_1, pattern_dist_2, patterns)
	return (ntpath.basename(file)[:8], pd.to_numeric(records['theta']).mean(),\
	 pd.to_numeric(records['distance']).mean(), pattern_theta_1, pattern_theta_2,\
	 pattern_dist_1, pattern_dist_2, patterns)
...
if __name__ == '__main__':
	...
	if int(args.search_mode) == 0:
		...
		print('keys we are testing on: {}'.format(len(keys)))
		for file in files:	    
			#If reading with pandas
			if iskeys_file == 1:
				df = pd.read_table(file, delimiter = '\t', names = column_names, dtype = 'unicode')
				summary.append(search_by_key(ntpath.basename(file)[:8], \
							df,\
							keys,1))
	...
	if int(args.search_mode) == 2:
		...
		#Change here Sarika
		#all_list = [['CYS', 'HIS', 'ASN']]#Wu change for three amino acid like an exmaple [['LEU', 'LEU','LEU']]
		print 'sarika2'
		print all_list
		print files
		already_completed = []
