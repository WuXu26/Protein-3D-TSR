...
def generate_all_others_together_df(all_others_together, all_files_in_others, group_percent_threshold):
    ...
    df_all_others_together=df_all_others_together[(df_all_others_together['group_percent']> 50 )\
...
def generate_box_plot(all_others_together, all_files_in_others, group_in_test):
    ...
    with open(os.path.join(args.path, args.sample_name, args.bin_folder, 'all_others_percents_{}'.format(group_in_test)), 'wb') as fp:
             pickle.dump(all_others_percents, fp)
    with open(os.path.join(args.path, args.sample_name, args.bin_folder, 'group_percents_{}'.format(group_in_test)), 'wb') as fp:
             pickle.dump(group_percents, fp)
...
if __name__ == '__main__':
	...
	groups_display_name = { "Receptor": "Receptor",
	                      "kinase": "kinase",
	                      "phosphatase": "phosphatase",
                              "protease": "protease",
	                      "isomerase": "isomerase"}
	top_n = 1
	x_axis_step = {'Receptor': 0.25, 'kinase': 0.85, 'phosphatase': 1, 'protease': 1, 'isomerase': 1}
	y_axis_step = {'Receptor': 300, 'kinase': 20, 'phosphatase': 5000, 'protease': 100, 'isomerase': 100}
	...
	for group in groups:
		...
		####Add this line--Sarika####
		df_all_others_together.to_csv(os.path.join(args.path, args.sample_name, args.bin_folder, 'a_not_b_{}.csv'.format(group)))
