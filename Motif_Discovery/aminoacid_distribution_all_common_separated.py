...
def calculate_statistics(file, is_freq_groups, fileType):
		...
		aacd0_no_grouping = Counter(df['aa0'].values)
		aacd1_no_grouping = Counter(df['aa1'].values)
		aacd2_no_grouping = Counter(df['aa2'].values)
		aminoacid_freq_no_grouping = aacd0_no_grouping + aacd1_no_grouping + aacd2_no_grouping
		aminoacid_freq_no_grouping = pd.DataFrame(aminoacid_freq_no_grouping.items(), columns = ['aacd', 'sum'])
		...
		return aminoacid_freq_no_grouping, \
			all_mean_theta, all_mean_dist, \
			all_std_theta, all_std_dist, \
			all_mean_freq, all_std_freq
...
def generate_common_key_files(all_keys_files, outFolder):
	...
	if os.path.exists(os.path.join(args.path, args.sample_name, args.setting, \
						'common_keys')):
		print('generate_common_key_files() : Common Key files already exist from \
		 a previous program.Check this flow.')
		...
		if(len(common_triplet_files) != len(all_keys_files)):
			print("ERROR: generate_common_key_files() : All proteins did \
				not have common keys triplet \
				files generated. Repeat common key files calculation. Aborting..")
			return
		...
		print('generate_common_key_files() : Common Key files not present. \
			Generating common key files.')
