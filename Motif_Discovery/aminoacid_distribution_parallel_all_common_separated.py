...
parser.add_argument('--onlyTriplets', '-onlyTriplets', \
    action='store_true', default=False, \
    help='Enable this option if aa percentis required for only triplet files')
...
def search_by_key(fileName, file, keys, iskeys_file, outFolder):
	...
	if os.path.exists(os.path.join(outFolder,'{}_common_keys.csv'.format(fileName))) and \
		...
		#print('search_by_key() : Skipping as {}_common_key.csv and {}_uncommon_key.csv exists. '.format(ntpath.basename(file)[:4]))
		return
...
def calculate_statistics(file, fileType):
		...
		aminoacid_freq_no_grouping = pd.DataFrame(\
			(Counter(df['aa0'].values) \
			+ Counter(df['aa1'].values) \
			+ Counter(df['aa2'].values)).items()\
			, columns = ['aacd', 'sum'])
		aminoacid_freq_no_grouping['percent'] = \
		100* aminoacid_freq_no_grouping['sum']/aminoacid_freq_no_grouping['sum'].sum()

		aminoacid_freq_no_grouping.to_excel(writer,sheet_name='all_aa_distribution')
		writer.close()

		return dict(dict(zip(aminoacid_freq_no_grouping.aacd, aminoacid_freq_no_grouping.percent)),\
			**{'all_mean_theta': df['theta'].mean(), 'all_mean_dist': df['distance'].mean(), \
				'all_std_theta':df['theta'].std(), 'all_std_dist': df['distance'].std(), \
				'all_mean_freq': np.mean(Counter(df['key'].values).values()), \
				'all_std_freq':np.mean(Counter(df['key'].values).values()),\
				'protein': ntpath.basename(file)[:4]})

def get_aa_percents(keys_files, fileType):
	"""Get amino acid percentages for all files in the specified group parallelely."""
	print('get_aa_percents() : Parallel amino acid percentage generation')				
	return Parallel(n_jobs=cpu_count() - 1, verbose=10, \
		backend="multiprocessing", batch_size="auto")\
                (delayed(calculate_statistics)(file, fileType) for file in keys_files)
...
def generate_common_key_files(all_keys_files, outFolder):
	...
	if os.path.exists(os.path.join(args.path, args.sample_name, args.setting, \
						'common_keys')):
		print("generate_common_key_files() :", end = '')
		print("Common Key files already exist from a previous program.Check this flow.")
		common_triplet_files = glob.glob(os.path.join(os.path.join(args.path, \
			args.sample_name, args.setting, \
						'common_keys'), '*_key_search_common_keys.csv'))
		if(len(common_triplet_files) != len(all_keys_files)):
			print("ERROR: generate_common_key_files() : ", end = '')
			print("All proteins did not have common keys triplet files generated.", end = '')
			print("Repeat common key files calculation. Aborting..")
			return
		
		for file in common_triplet_files:
			if os.path.exists(os.path.join(outFolder,\
				'{}_common_keys.csv'.format(ntpath.basename(file)[:4]))):
				print('generate_common_key_files() : Skipping as {}_common_key.csv exists. '\
					.format(ntpath.basename(file)[:4]))
				continue
			os.rename(file, os.path.join(outFolder,\
				'{}_common_keys.csv'.format(ntpath.basename(file)[:4])))

	else:
		print("generate_common_key_files() : ", end = '')
		print("Common Key files not present.Generating common key files.")
		keys = common_keys(all_keys_files)
		pd.DataFrame(keys).to_csv(os.path.join(outFolder,'common_keys.csv'))

		merged_files = glob.glob(os.path.join(args.path, args.sample_name, \
			args.setting, '*.triplets*'))
		for file in merged_files:
			...
			if iskeys_file == 1:
				search_by_key(ntpath.basename(file)[:4], \
					...
					dtype = {'key': np.dtype(int), 'aa0': str, 'aa1': str,  'aa2': str, \
					'theta': np.dtype(float), 'distance': np.dtype(float)},\
					names = ['key', 'aa0', 'pos0', 'aa1', 'pos1', 'aa2', 'pos2', \
					'classT', 'theta', 'classL', 'distance', 'x0', 'y0', 'z0', 'x1', 'y1',\
					'z1', 'x2', 'y2', 'z2']),\
							keys,1, outFolder)
...
def generate_uncommon_key_files(all_keys_files, outFolder, is_freq_groups):
	"""Generates uncommon keys files under 'aa_percent_files' required for aminoacid percentage."""
	...
	if not un_common_key_files:
		...
		for file in all_keys_files:
			...
			m_file = os.path.join(setting_folder, '{}.triplets_{}'.format(file_name, args.setting))
			if not c_file:
				print('generate_uncommon_key_files() : {} doesnot have common key file generated. Aborting..'\
					.format(file_name))
				return
			c_keys = list(pd.read_table(c_file, delimiter = ',', header = 0,  \
					dtype = {'key': np.dtype(int), 'aa0': str,  \
					'aa1': str,  'aa2': str, 'theta': np.dtype(float), \
					'distance': np.dtype(float)}).key.unique())
			m_file = pd.read_table(m_file, delimiter = '\t', usecols = use_col_names, \
					dtype = {'key': np.dtype(int), 'aa0': str,  \
					'aa1': str,  'aa2': str, 'theta': np.dtype(float), \
