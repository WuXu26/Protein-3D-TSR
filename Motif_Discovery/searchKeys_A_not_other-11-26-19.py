...
def get_certain_percent_keys_groups():
	...
	for group in groups:
		...
		for group2 in groups:
			if group != group2:
				...
				df = pd.read_csv(os.path.join(args.path, args.sample_name, \
							args.setting, 'keys_percents_{}_{}.csv'.format(group2, 0)),header = 0, index_col = 0)
				df.columns =  ['key','#files_present_'+group2, 'key_percent_'+ group2]
		...
		print type(df_merged['key'].values)
...
def get_certain_percent_keys_groups_pie():
	writer = pd.ExcelWriter(os.path.join(args.path, args.sample_name, \
					args.setting, 'common_keys_group_specific.xlsx'), \
					engine = 'xlsxwriter')
	samples_file = pd.read_csv(os.path.join(args.path, \
